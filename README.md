...
- ### Логирование и распределенная трассировка (#ДЗ №18)
...
```
Мы используем стэк EFK:
```
ElasticSearch (TSDB и поисковый движок для хранения данных)
Fluentd (для агрегации и трансформации данных)
Kibana (для визуализации)
```
Создаем отдельный файл docker/docker-compose-logging.yml
Собран контейнер fluentd с конфигурацией для нашей системы cd logging/fluentd/ && docker build -t $USERNAME/fluentd .
...
tag: service.post
```
Протестировал визуализацию при помощи kibana
